...
As the tolerance level for the above model was a bit high, we are now going to try to tune the optimizer to see if this allows for a lower tolerance.
...
fit_var_6 <- glmer(repay_fail ~ int_rate + emp_length + annual_inc + purpose +
                     ...
                     (1|addr_state) + (1|earliest_cr_line) + (1|zip_code),
                   data = extended_scaled, family = binomial,
                   control = glmerControl(optimizer = "Nelder_Mead", check.conv.grad = .makeCC("warning", tol = 8e-3, relTol = NULL)))
```
