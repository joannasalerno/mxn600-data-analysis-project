...
prob_train=predict(fit_probit,type=c("response"))
g <- roc(train_scaled_data$repay_fail ~ prob_train)
AUC <- g$auc
AUC
Gini <- 2*(AUC - 1/2)
Gini
...
prob_val=predict(fit_probit,newdata = val_scaled_data,type=c("response"))
g <- roc(val_scaled_data$repay_fail ~ prob_val)
AUC <- g$auc
AUC
Gini <- 2*(AUC - 1/2)
Gini
...
Finally, we will test whether using a complementary log-log link function changes the AIC or predictive performance of our model.
...
confusion_glm <- ifelse(prob_val > 0.5, "1", "0")
...
In terms of our final model's predictive performance, it does much better than the credit risk model the bank was previously using. While the previous models yielded a $Gini=0.114$ on the training dataset and a $Gini=0.110$ on the validation (unseen) dataset, our models produced a $Gini=0.410$ on the training dataset and a $Gini=0.395$ on the validation dataset.
...
*** ADD EFFECTS AND CIS AND P-VALUES TO BELOW LIST
...
*** EXPLAIN WHAT THE CHOSEN MODEL IS

## Analyse Performance Benchmarks

Now, before we begin to interpret these extended models, we will first analyse performance benchmarks, or goodness-of-fit for the data. We'll start by looking at the model with all separate random effects (time, state, and zip code as random effects).
...
As can be seen above, while this model does not indicate overdispersion of significant outliers, there appears to be significant deviation from normality, as seen by the significant KS test results on the left. Let's now consider the model with just 2 random effects: time and location, in which location consists of zip codes nested within states.
...
As can be seen in the above plots, this model that includes location as a nested random effect of zip codes within states appears to be a better fit for the data. Now, there is no indication of significant deviations from normality, nor overdisperion or outliers. Thus, we will select this model to further analyse. Now we can look more into the residuals of the selected model.
...
require(caret)
cm <- confusionMatrix(data=confusion_glmm,
...
In terms of the selected model's random effects:

location: The location random effect captures the variation in the response variable that is associated with different locations. The estimated standard deviation for the location random effect is approximately 0.2261. The estimated variance (0.05114) express the degree of variation between different locations, while the standard deviation provides a measure of this variation.

earliest_cr_line: The earliest_cr_line random effect captures the variation in the response variable associated with the year of an individual's earliest credit line. The estimated standard deviation for the earliest_cr_line random effect is approximately 0.1189. The estimated variance (0.01414) represents the degree of variation between different earliest credit lines, and the standard deviation (0.1189) express the magnitude of this variation.
...
*** ADD EFFECTS, CIS, P-VALUES BELOW
