...
```{r remove variable X from all datasets}
train_data <- train_data[, -1]
val_data <- val_data[, -1]
test_data <- test_data[, -1]
extended <- extended[, -1]
```
Variable X which is the number of each record is removed from all datasets.
...
fit_logit <- glm(repay_fail ~ int_rate + emp_length + annual_inc + purpose + inq_last_6mths + pub_rec + revol_bal + revol_util + term * int_rate,
...
# fit_logit_val <- glm(repay_fail ~ int_rate + emp_length + annual_inc + purpose + inq_last_6mths + pub_rec + revol_bal + revol_util + term * int_rate,
#     data = val_data, family = binomial)


pred.cv <- rep(0,80)
for(i in 1:80){
fit.cv <- glmer(guard~ttt+(1|block),data=new_culcita[-i,],
             family=binomial)
pred.cv[i] <- predict(fit.cv,newdata=new_culcita[i,])
}
pred.cv <- exp(pred.cv)/(1+exp(pred.cv))
ind <- which(pred.cv>0.625)
ycv <- rep(0,80)
ycv[ind] <- 1
table(new_culcita$guard,ycv)

# pred <- as.numeric()
# for(i in 1:length(val_data)){
# temp <- val_data[-i,]
# yntemp <- repay_fail[-i]
# fit.cv <- glm(yntemp~temp,family="binomial",data=temp)
# predlp <- predict(fit.cv,newdata=data.frame(temp=val_data$temp[i]))
# pred[i] <- exp(predlp)/(1+exp(predlp))*repay_fail[i]
# }

# # Need to do some cross-validation
# pred <- as.numeric()
# for(i in 1:length(y)){
# temp <- data.frame(y=y[-i],n=n[-i],dose=dose[-i],d2=d2[-i])
# yntemp <- cbind(temp$y,temp$n-temp$y)
# fit.cv <- glm(yntemp~dose + d2,family="binomial",data=temp)
# predlp <- predict(fit.cv,newdata=data.frame(dose=dose[i],d2=d2[i]))
# pred[i] <- exp(predlp)/(1+exp(predlp))*n[i]
# }
...
fit_probit_val <- glm(repay_fail ~ int_rate + emp_length + annual_inc + purpose + inq_last_6mths + pub_rec + revol_bal + revol_util + term * int_rate,
    data = val_data, family = binomial("probit"))

#ROC curve on validation data 
prob=predict(fit_probit_val,type=c("response"))
g <- roc(val_data$repay_fail ~ prob)
AUC <- g$auc
AUC
Gini <- 2*(AUC - 1/2)
