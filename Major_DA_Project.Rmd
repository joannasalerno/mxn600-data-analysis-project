...
Based on Five Cs Credit information and considering Correlation between variales below are the variables more important to assess the risk whether the creditor will default on a loan:

repay_fail ~ annual_inc * emp_length * verification_status * dti * credit_age_yrs * home_ownership * 
...
```{r}
fit_val <- glm(formula = repay_fail ~ term * int_rate + emp_length + home_ownership + annual_inc + purpose + inq_last_6mths +                  pub_rec + revol_bal + revol_util,
            data = val_data,
            family = "binomial"
    )
