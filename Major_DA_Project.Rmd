...
```{r}
numeric_vars <- c("loan_amnt", "annual_inc", "dti", "delinq_2yrs", "inq_last_6mths", "open_acc", "pub_rec", "revol_util", "revol_bal", "total_acc", "credit_age_yrs")
par(mfrow=c(3,3))
for (var in numeric_vars) {
  hist(training_data[[var]], main=var, xlab=var)
}

ggplot(training_data, aes(x = purpose)) +
  geom_bar() +
  labs(x = "Purpose", y = "Frequency") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(training_data, aes(x = term, y = loan_amnt)) +
  geom_boxplot() +
...
```{r corr_matrix}
columns <- c("loan_amnt", "annual_inc", "dti", "delinq_2yrs", "inq_last_6mths", "open_acc", "pub_rec", "revol_util", "revol_bal", "total_acc", "credit_age_yrs")
