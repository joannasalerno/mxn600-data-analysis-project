...
#names(train_scaled_data)
#summary(train_scaled_data)
```

Stepwise AIC technique is used to find the best model:

```{r stepwiseAIC for GLM}
# 
# #Full model with all possible interactions for backwards selection:
# full_interaction_model <- glm(data = train_scaled_data,
#     formula = repay_fail ~ annual_inc2 * emp_length * verification_status * dti * credit_age_yrs * home_ownership *
#       total_acc * revol_bal2 * int_rate * term * purpose * loan_amnt2 * inq_last_6mths * delinq_2yrs,
#     family = "binomial")
# 
# #Model with no variables present for forwards selection:
...
fit_logit <- glm(data = train_scaled_data,
...
```{r}
fit_logit_val <- glm(formula = repay_fail ~ annual_inc + emp_length + verification_status + dti + credit_age_yrs + home_ownership + 
      total_acc + revol_bal + int_rate + term + purpose + loan_amnt + inq_last_6mths + delinq_2yrs,
            data = val_data,
            family = "binomial"
    )
...
prob=predict(fit_logit_val,type=c("response"))
