...
At first glance of the first 10 rows of this dataset, we notice there is a value of n/a in the employment length column. There are also a few zero values in a few columns. This will prompt us to further explore the data for any true missing values.
...
```{r string_na_rows}
train_data %>% filter_all(any_vars(. %in% c('n/a')))
```
